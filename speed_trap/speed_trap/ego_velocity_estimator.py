...
def ego_velocity_estimator():
    ...
    velocity_publisher  = rospy.Publisher("ego_velocity", Vector3, queue_size=20)
...
def odom_callback(msg : Odometry):
    ...
    ego_velocity.x = (msg.pose.pose.position.x - previous_pose.x) / time_difference
    ego_velocity.y = (msg.pose.pose.position.y - previous_pose.y) / time_difference
    ego_velocity.z = (msg.pose.pose.position.z - previous_pose.z) / time_difference
    previous_timestamp = datetime.fromtimestamp(int(str(msg.header.stamp.secs) + str(msg.header.stamp.nsecs)) / 1e9)
    previous_pose = msg.pose.pose.position
